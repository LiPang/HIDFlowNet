...
class LMDBDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        with env.begin(write=False) as txn:
            image_byte = txn.get('{:08}'.format(index).encode('ascii'))
            image = pickle.loads(image_byte)
...
def make_dataset(opt, train_transform, target_transform, common_transform, batch_size=None, repeat=1):
    ...
    if suffix == 'db':
        ...
        dataset = NPZDataset(opt.dataroot, repeat=repeat)
    else:
    ...
    train_loader = torch.utils.data.DataLoader(train_dataset,
                              batch_size=batch_size or opt.batchSize, shuffle=False,
