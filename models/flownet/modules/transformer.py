...
class LinearProjection(nn.Module):
    ...
    def forward(self, x, attn_kv=None):
        ...
        kv = self.to_kv(attn_kv).reshape(B_, N, 2, self.heads, self.inner_dim // self.heads).permute(2, 0, 3, 1, 4)
        q = q[0]
        k, v = kv[0], kv[1]
        return q, k, v
...
class WindowAttention(nn.Module):
    def __init__(self, dim, hidden_dim=60, win_size=8, num_heads=4, token_projection='linear', qkv_bias=True, qk_scale=None, attn_drop=0.,
        ...
        if token_projection == 'conv':
            self.qkv = ConvProjection(dim, num_heads, hidden_dim // num_heads, bias=qkv_bias)
        elif token_projection == 'linear_concat':
            self.qkv = LinearProjection_Concat_kv(dim, num_heads, hidden_dim // num_heads, bias=qkv_bias)
        else:
            self.qkv = LinearProjection(dim, num_heads, hidden_dim // num_heads, bias=qkv_bias)
    ...
    def forward(self, x, attn_kv=None, mask=None):
        ...
        x = (attn @ v).transpose(1, 2).reshape(B_, N, self.hidden_dim)
        x = self.proj(x)
        x = self.se_layer(x)
        x = self.proj_drop(x)
        return x
...
class LeWinTransformerBlock(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        #     attn_mask = attn_mask + self.shift_attn_mask if attn_mask is not None else self.shift_attn_mask
        ...
        # FFN
        x_mid1 = self.drop_path(x)
        x = shortcut + x_mid1
        x_mid2 = self.drop_path(self.mlp(self.norm2(x)))
        x = x + x_mid2
        del attn_mask

        # N H*W C -> N C H W
        x = x.transpose(1, 2).reshape(B, -1, H, W).contiguous()
        x_mid1, x_mid2 = x_mid1.transpose(1, 2).reshape(B, -1, H, W).contiguous(),\
                         x_mid2.transpose(1, 2).reshape(B, -1, H, W).contiguous()
        return x, x_mid1, x_mid2
...
class LFormer(nn.Module):
    def __init__(self, in_features, mid_features, n_blocks):
        super(LFormer, self).__init__()
        self.blocks = nn.ModuleList()
        self.conv_first = nn.Conv2d(in_features, mid_features, 3, 1, 1)
        for i in range(n_blocks):
            self.blocks.append(BasicUformerLayer(
                dim=mid_features,
        ...
        self.conv_last = nn.Conv2d(mid_features, in_features, 3, 1, 1)
